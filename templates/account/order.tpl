    ...
    <div class="container" data-store="{{ detail_data_store }}">
        {% if is_subscription %}
            {% set subscription_action_success_keys = {
                'skip': 'our_components.customer_subscriptions.skip_success',
                'address': 'our_components.customer_subscriptions.address_success',
                'cancel': 'our_components.customer_subscriptions.cancel_success',
            } %}
            {% set success_key = subscription_action_success_keys[subscription_action_success] ?? null %}
            {% if success_key %}
            ...
            {% elseif subscription_action_error %}
                <div class="alert alert-danger mt-0 mb-3">
                    {{ 'our_components.customer_subscriptions.action_error' | tt }}
                </div>
            {% endif %}
